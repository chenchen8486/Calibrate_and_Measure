...
　　变更记录：新增返回码 AI_MISS(5)。AI 分割链路中，板上有物体但模型认不出目标产品时，旧版会静默退回传统粗掩膜，把杂物当产品输出一组假宽高，本版改为直接返回 AI_MISS 且不给任何测量数据，由软件提示人工复核。同时给 Init / Measure / SetBackground 三个入口补全异常兜底：任何内部异常（含未知异常）都捕获为 INTERNAL 错误码或 false+errMsg 返回，接口不再向外抛异常（旧版异常会抛给调用方，宿主程序表现为无返回、卡死或闪退）。

### 要替换的源码文件（7 个，替换后须重新编译）
...
| `src/cam_api.h` | 错误模型注释更新，版本号 2.1.4 |
| `src/cam_api.cpp` | Init / Measure / SetBackground 全面 try/catch，版本号 2.1.4 |
...
| 第 7 节 集成注意事项 | 三处：AI_MISS 的处理姿势、`NO_PRODUCT` 与 `AI_MISS` 的区别（传统链路不产生 AI_MISS）、异常一律转为错误码返回 | 按该节把三个返回码分支补齐，无返回/卡死问题时照该节排查 DLL |
| 第 9 节 依赖与分发清单 | 运行时 dll 必须放 exe 同目录的原因说明 | 核对部署目录下的 `onnxruntime.dll` 等文件版本与位置 |

### 替换后要做的事

　　重新编译（Release x64），并按上节手工合并 config.ini 的 debug 开关。软件代码不改也能编译通过。建议软件对 AI_MISS 做分支处理：收到后按“疑似放错物料或模型失效”提示人工复核，本帧不要触发任何硬件调整。另请核对部署目录：onnxruntime.dll 等运行时库必须与 exe 同目录，System32 或 PATH 里的旧版同名 dll 会被 Windows 优先加载，导致初始化崩溃且无任何错误码返回（本机已实测复现）。背景缓存 temp/background_model.bmp 已同步为当前场景空板图，随仓库更新即可，场景变更后按 README 6.1 节重学替换。
