...
　　说明：本轮不改算法与接口行为，只动部署默认值和版本号。

### 一、要替换的源码文件（2 个，仅版本号串变化）

| 文件 | 变化说明 |
|---|---|
| `src/cam_api.h` | 版本号串 2.1.4 → 2.1.5 |
| `src/cam_api.cpp` | 版本号串 2.1.4 → 2.1.5 |

　　不关心版本串的话这两个文件可以不换，功能没有差别。
...
　　本版未改 README。上一版（2.1.4）涉及的三处阅读指引见其条目第三节。
...
1. 按第二节手工改 `config.ini` 的那一项，只改这一项，勿整文件覆盖。
2. 用新版覆盖第一节表中的两个源码文件，Release x64 重新编译。不改版本串可跳过本步。
3. 核对 exe 同目录的 `onnxruntime.dll`、`opencv_world480.dll` 是否为随包版本。System32 或 PATH 里的旧版同名 dll 会被 Windows 优先加载，导致初始化崩溃且无任何错误码返回。

---

## v2.1.4（2026-09-22）

　　一句话：AI 分割漏检不再静默兜底出假数据，改报专用错误码 `AI_MISS`，三个对外接口的内部异常一律转为错误码返回。
...
| `src/cam_api.cpp` | Init / Measure / SetBackground 三个入口全面 try/catch，版本号 2.1.4 |
...
2. Release x64 重新编译。
3. 核对 exe 同目录的 `onnxruntime.dll`、`opencv_world480.dll` 是否为随包版本。System32 或 PATH 里的旧版同名 dll 会被 Windows 优先加载，导致初始化崩溃且无任何错误码返回（本机已实测复现）。
4. 背景缓存 `temp/background_model.bmp` 已同步为当前场景空板图，克隆仓库即带最新版。现场换场景后按 README 6.1 节用 SetBackground 重学。
